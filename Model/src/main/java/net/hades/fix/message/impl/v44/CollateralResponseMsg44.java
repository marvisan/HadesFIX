...
import net.hades.fix.message.group.impl.v44.UndInstrmtCollGroup44;
...
/**
 * FIX version 4.4 CollateralResponseMsg implementation.
 *
 * @author <a href="mailto:support@marvisan.com">Support Team</a>
 * @version $Revision: 1.3 $
 * @created 19/12/2011, 9:32:41 AM
 */
@XmlRootElement(name="CollRsp")
@XmlType(propOrder = {"header", "partyIDGroups", "execCollGroups", "trdCollGroups", "instrument", "financingDetails", "instrumentLegs", "undInstrmtCollGroups",
    "trdRegTimestampsGroups", "miscFeeGroups", "spreadOrBenchmarkCurveData", "stipulationsGroups"})
@XmlAccessorType(XmlAccessType.NONE)
public class CollateralResponseMsg44 extends CollateralResponseMsg {
    ...
    protected static final Set<Integer> UND_INSTRMT_COLL_GROUP_TAGS = new UndInstrmtCollGroup44().getFragmentAllTags();
    ...
    static {
        ...
        ALL_TAGS.addAll(UND_INSTRMT_COLL_GROUP_TAGS);
        ...
        START_COMP_TAGS.addAll(UND_INSTRMT_COLL_GROUP_TAGS);
    ...
    public CollateralResponseMsg44() {
        super();
    }

    public CollateralResponseMsg44(Header header, ByteBuffer rawMsg)
    throws InvalidMsgException, TagNotPresentException, BadFormatMsgException {
        super(header, rawMsg);
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }

    public CollateralResponseMsg44(BeginString beginString) throws InvalidMsgException {
        super(beginString);
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }

    public CollateralResponseMsg44(BeginString beginString, ApplVerID applVerID) throws InvalidMsgException {
    ...
    public void copyFixmlData(FIXFragment fragment) {
        CollateralResponseMsg44 fixml = (CollateralResponseMsg44) fragment;
        if (fixml.getCollRespID() != null) {
            collRespID = fixml.getCollRespID();
        }
        if (fixml.getCollAsgnID() != null) {
            collAsgnID = fixml.getCollAsgnID();
        }
        if (fixml.getCollReqID() != null) {
            collReqID = fixml.getCollReqID();
        }
        if (fixml.getCollAsgnReason() != null) {
            collAsgnReason = fixml.getCollAsgnReason();
        }
        if (fixml.getCollAsgnTransType() != null) {
            collAsgnTransType = fixml.getCollAsgnTransType();
        }
        if (fixml.getCollAsgnRespType() != null) {
            collAsgnRespType = fixml.getCollAsgnRespType();
        }
        if (fixml.getCollAsgnRejectReason() != null) {
            collAsgnRejectReason = fixml.getCollAsgnRejectReason();
        }
        ...
        if (fixml.getUndInstrmtCollGroups() != null && fixml.getUndInstrmtCollGroups().length > 0) {
            setUndInstrmtCollGroups(fixml.getUndInstrmtCollGroups());
        }
    ...
    @Override
    public CollAsgnReason getCollAsgnReason() {
        return collAsgnReason;
    }

    @Override
    public void setCollAsgnReason(CollAsgnReason collAsgnReason) {
        this.collAsgnReason = collAsgnReason;
    }

    @XmlAttribute(name = "TransTyp")
    @Override
    public CollAsgnTransType getCollAsgnTransType() {
        return collAsgnTransType;
    }

    @Override
    public void setCollAsgnTransType(CollAsgnTransType collAsgnTransType) {
        this.collAsgnTransType = collAsgnTransType;
    }

    @XmlAttribute(name = "RespTyp")
    @Override
    public CollAsgnRespType getCollAsgnRespType() {
        return collAsgnRespType;
    }

    @Override
    public void setCollAsgnRespType(CollAsgnRespType collAsgnRespType) {
        this.collAsgnRespType = collAsgnRespType;
    }

    @XmlAttribute(name = "RejRsn")
    @Override
    public CollAsgnRejectReason getCollAsgnRejectReason() {
        return collAsgnRejectReason;
    }

    @Override
    public void setCollAsgnRejectReason(CollAsgnRejectReason collAsgnRejectReason) {
    ...
    public void setNoUnderlyings(Integer noUnderlyings) {
        ...
        if (noUnderlyings != null) {
            undInstrmtCollGroups = new UndInstrmtCollGroup[noUnderlyings.intValue()];
            FragmentContext context = new FragmentContext(sessionCharset, messageEncoding, encryptionRequired, crypter, validateRequired);
            for (int i = 0; i < undInstrmtCollGroups.length; i++) {
                undInstrmtCollGroups[i] = new UndInstrmtCollGroup44(context);
            }
        }
    }
    ...
    @XmlElementRef
    @Override
    public UndInstrmtCollGroup[] getUndInstrmtCollGroups() {
        return undInstrmtCollGroups;
    }

    public void setUndInstrmtCollGroups(UndInstrmtCollGroup[] underlyingInstruments) {
        this.undInstrmtCollGroups = underlyingInstruments;
    ...
    @Override
    public UndInstrmtCollGroup addUndInstrmtCollGroup() {
        UndInstrmtCollGroup group = new UndInstrmtCollGroup44(new FragmentContext(sessionCharset, messageEncoding, encryptionRequired, crypter, validateRequired));
        List<UndInstrmtCollGroup> groups = new ArrayList<UndInstrmtCollGroup>();
        if (undInstrmtCollGroups != null && undInstrmtCollGroups.length > 0) {
            groups = new ArrayList<UndInstrmtCollGroup>(Arrays.asList(undInstrmtCollGroups));
        }
        groups.add(group);
        undInstrmtCollGroups = groups.toArray(new UndInstrmtCollGroup[groups.size()]);
        noUnderlyings = new Integer(undInstrmtCollGroups.length);

        return group;
    }

    @Override
    public UndInstrmtCollGroup deleteUndInstrmtCollGroup(int index) {
        UndInstrmtCollGroup result = null;
        if (undInstrmtCollGroups != null && undInstrmtCollGroups.length > 0 && undInstrmtCollGroups.length > index) {
            List<UndInstrmtCollGroup> groups = new ArrayList<UndInstrmtCollGroup>(Arrays.asList(undInstrmtCollGroups));
            result = groups.remove(index);
            undInstrmtCollGroups = groups.toArray(new UndInstrmtCollGroup[groups.size()]);
            if (undInstrmtCollGroups.length > 0) {
                noUnderlyings = new Integer(undInstrmtCollGroups.length);
            } else {
                undInstrmtCollGroups = null;
                noUnderlyings = null;
            }
        }
    ...
    @Override
    public int clearUndInstrmtCollGroups() {
        int result = 0;
        if (undInstrmtCollGroups != null && undInstrmtCollGroups.length > 0) {
            result = undInstrmtCollGroups.length;
            undInstrmtCollGroups = null;
            noUnderlyings = null;
        }
    ...
    protected void validateRequiredTags() throws TagNotPresentException {
        ...
        if (collRespID == null || collRespID.trim().isEmpty()) {
            errorMsg.append(" [CollRespID]");
            hasMissingTag = true;
        }
        if (collAsgnID == null || collAsgnID.trim().isEmpty()) {
            errorMsg.append(" [CollAsgnID]");
            hasMissingTag = true;
        }
        if (collAsgnReason == null) {
            errorMsg.append(" [CollAsgnReason]");
            hasMissingTag = true;
        }
        if (collAsgnRespType == null) {
            errorMsg.append(" [CollAsgnRespType]");
            hasMissingTag = true;
        }
    ...
    protected byte[] encodeFragmentSecured(boolean secured) throws TagNotPresentException, BadFormatMsgException {
        ...
        try {
            if (MsgUtil.isTagInList(TagNum.CollRespID, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.CollRespID, collRespID);
            }
            if (MsgUtil.isTagInList(TagNum.CollAsgnID, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.CollAsgnID, collAsgnID);
            }
            if (MsgUtil.isTagInList(TagNum.CollReqID, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.CollReqID, collReqID);
            }
            if (collAsgnReason != null && MsgUtil.isTagInList(TagNum.CollAsgnReason, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.CollAsgnReason, collAsgnReason.getValue());
            }
            if (collAsgnTransType != null && MsgUtil.isTagInList(TagNum.CollAsgnTransType, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.CollAsgnTransType, collAsgnTransType.getValue());
            }
            if (collAsgnRespType != null && MsgUtil.isTagInList(TagNum.CollAsgnRespType, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.CollAsgnRespType, collAsgnRespType.getValue());
            }
            if (collAsgnRejectReason != null && MsgUtil.isTagInList(TagNum.CollAsgnRejectReason, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.CollAsgnRejectReason, collAsgnRejectReason.getValue());
            }
            ...
            if (noUnderlyings != null && MsgUtil.isTagInList(TagNum.NoUnderlyings, SECURED_TAGS, secured)) {
                ...
                if (undInstrmtCollGroups != null && undInstrmtCollGroups.length == noUnderlyings.intValue()) {
                    for (int i = 0; i < noUnderlyings.intValue(); i++) {
                        if (undInstrmtCollGroups[i] != null) {
                            bao.write(undInstrmtCollGroups[i].encode(getMsgSecureTypeForFlag(secured)));
                        }
                    }
                } else {
                    String error = "UndInstrmtCollGroup field has been set but there is no data or the number of groups does not match.";
                    LOGGER.severe(error);
                    throw new BadFormatMsgException(SessionRejectReason.IncorrectCountForGroups, getHeader().getMsgType(),
    ...
    throws BadFormatMsgException, InvalidMsgException, TagNotPresentException {
        ...
        if (UND_INSTRMT_COLL_GROUP_TAGS.contains(tag.tagNum)) {
            if (noUnderlyings != null && noUnderlyings.intValue() > 0) {
                message.reset();
                undInstrmtCollGroups = new UndInstrmtCollGroup[noUnderlyings.intValue()];
                for (int i = 0; i < noUnderlyings.intValue(); i++) {
                    UndInstrmtCollGroup component = new UndInstrmtCollGroup44(context);
                    component.decode(message);
                    undInstrmtCollGroups[i] = component;
                }
            }
        }
    ...
    protected String getUnsupportedTagMessage() {
        return "This tag is not supported in [CollateralRequestMsg] message version [4.4].";
    }

...
/*
 * NestedParties344.java
 *
 * $Id: NestedParties344.java,v 1.2 2011-04-14 23:44:59 vrotaru Exp $
 */
...
import net.hades.fix.message.group.impl.v44.Nested3PartyGroup44;
import net.hades.fix.message.type.TagNum;
import net.hades.fix.message.util.TagEncoder;

/**
 * FIX version 4.4 implementation of NestedParties3 component.
 *
 ...
 */
@XmlTransient
public class NestedParties344 extends NestedParties3 {
    ...
    protected static final Set<Integer> NESTED3_PARTY_GROUP_TAGS = new Nested3PartyGroup44().getFragmentAllTags();
    ...
    public NestedParties344() {
        super();
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }

    public NestedParties344(FragmentContext context) {
        super(context);
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }
    ...
    public void setNoNested3PartyIDs(Integer noNested3PartyIDs) {
        ...
        if (noNested3PartyIDs != null) {
            ...
            for (int i = 0; i < nested3PartyGroups.length; i++) {
                nested3PartyGroups[i] = new Nested3PartyGroup44(new FragmentContext(sessionCharset, messageEncoding, encryptionRequired, crypter, validateRequired));
            }
        }
    }
    ...
    public Nested3PartyGroup addNested3PartyGroup() {
        Nested3PartyGroup group = new Nested3PartyGroup44(new FragmentContext(sessionCharset, messageEncoding, encryptionRequired, crypter, validateRequired));
    ...
    protected byte[] encodeFragmentSecured(boolean secured) throws TagNotPresentException, BadFormatMsgException {
        ...
        try {
            if (MsgUtil.isTagInList(TagNum.NoNested3PartyIDs, SECURED_TAGS, secured)) {
                ...
                if (nested3PartyGroups != null && nested3PartyGroups.length == noNested3PartyIDs.intValue()) {
                    for (int i = 0; i < noNested3PartyIDs.intValue(); i++) {
                        if (nested3PartyGroups[i] != null) {
                            bao.write(nested3PartyGroups[i].encode(getMsgSecureTypeForFlag(secured)));
                        }
                    }
                } else {
    ...
    throws BadFormatMsgException, InvalidMsgException, TagNotPresentException {
        if (NESTED3_PARTY_GROUP_TAGS.contains(tag.tagNum)) {
            if (noNested3PartyIDs != null && noNested3PartyIDs.intValue() > 0) {
                ...
                for (int i = 0; i < nested3PartyGroups.length; i++) {
                    Nested3PartyGroup group = new Nested3PartyGroup44(context);
    ...
    protected String getUnsupportedTagMessage() {
        return "This tag is not supported in [NestedParties3] component version [4.4].";
    }

 ...
 *
 * $Id: NewOrderSingleMsg.java,v 1.10 2011-04-28 10:07:47 vrotaru Exp $
 */
 ...
 * The new order message type is used by institutions wishing to electronically submit securities and
 * FOREX orders to a broker for execution. The New Order message type may also be used by
 * institutions or retail intermediaries wishing to electronically submit Collective Investment Vehicle (CIV)
 * orders to a broker or fund manager for execution.<br/>
 * Orders can be submitted with special handling instructions and execution instructions.
 * Handling instructions refer to how the broker should handle the order on its trading floor
 * (see HandlInst field). Execution instructions contain explicit directions as to how the order
 * should be executed (see ExecInst field).<br/>
 * New Order messages received with the PossResend flag set in the header should be validated by ClOrdID.
 * Implementations should also consider checking order parameters (side, symbol, quantity, etc.) to determine
 * if the order had been previously submitted. PossResends previously received should be acknowledged
 * back to the client via an Execution - Status message. PossResends not previously received should
 * be processed as a new order and acknowledged via an Execution - New message.<br/>
 ...
 * communication problems).To support FOREX accommodation trades, two fields, ForexReq and SettlCurrency,
 * are included in the message. To request a broker to execute a FOREX trade in conjunction with the
 * securities trade, the institution would set the ForexReq = Y and SettlCurrency = “intended settlement currency”.
 * The broker would then execute a FOREX trade from the execution currency to the settlement currency and
 * report the results via the execution message in the SettlCurrAmt and SettlCurrency fields.
 *
...
public abstract class NewOrderSingleMsg extends FIXMsg {
    ...
    /**
     * TagNum = 152. Starting with 4.1 version.
     */
    ...
    /**
     * TagNum = 23. Starting with 4.0 version.
     */
    protected String IOIID;

    /**
     * TagNum = 117. Starting with 4.0 version.
     */
    ...
    public NewOrderSingleMsg(Header header, ByteBuffer rawMsg)
    throws InvalidMsgException, TagNotPresentException, BadFormatMsgException {
        super(header, rawMsg);
    }

    public NewOrderSingleMsg(BeginString beginString) throws InvalidMsgException {
        super(MsgType.NewOrderSingle.getValue(), beginString);
    }

    public NewOrderSingleMsg(BeginString beginString, ApplVerID applVerID) throws InvalidMsgException {
        super(MsgType.NewOrderSingle.getValue(), beginString, applVerID);
    }
    ...
    protected void setFragmentTagValue(Tag tag) throws BadFormatMsgException {
        ...
        switch (tagNum) {
           ...
           case QuoteID:
            ...
            default:
                String error = "Tag value [" + tag.tagNum + "] not present in [NewOrderSingleMsg] fields.";
                LOGGER.severe(error);
    ...
    public String toString() {
        StringBuilder b = new StringBuilder("{NewOrderSingleMsg=");

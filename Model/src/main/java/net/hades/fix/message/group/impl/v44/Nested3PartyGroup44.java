...
/*
 * Nested3PartyGroup44.java
 *
 * $Id: Nested3PartyGroup44.java,v 1.2 2011-04-14 23:44:46 vrotaru Exp $
 */
...
public class Nested3PartyGroup44 extends Nested3PartyGroup {
    ...
    protected static final Set<Integer> NSTD3_PARTY_SUB_GROUP_TAGS = new Nested3PartySubGroup44().getFragmentAllTags();
    ...
    public Nested3PartyGroup44() {
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }

    public Nested3PartyGroup44(FragmentContext context) {
        super(context);
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }
    ...
    public void setNoNested3PartySubIDs(Integer noNested3PartySubIDs) {
        ...
        if (noNested3PartySubIDs != null) {
            ...
            for (int i = 0; i < nstd3PtysSubGroups.length; i++) {
                nstd3PtysSubGroups[i] = new Nested3PartySubGroup44(new FragmentContext(sessionCharset, messageEncoding, encryptionRequired, crypter, validateRequired));
            }
        }
    }
    ...
    public Nested3PartySubGroup addNstd3PtysSubGroup() {
        Nested3PartySubGroup group = new Nested3PartySubGroup44(new FragmentContext(sessionCharset, messageEncoding, encryptionRequired, crypter, validateRequired));
    ...
    protected byte[] encodeFragmentSecured(boolean secured) throws TagNotPresentException, BadFormatMsgException {
        ...
        try {
            if (MsgUtil.isTagInList(TagNum.Nested3PartyID, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.Nested3PartyID, nested3PartyID);
            }
            if (nested3PartyIDSource != null && MsgUtil.isTagInList(TagNum.Nested3PartyIDSource, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.Nested3PartyIDSource, nested3PartyIDSource.getValue());
            }
            if (nested3PartyRole != null && MsgUtil.isTagInList(TagNum.Nested3PartyRole, SECURED_TAGS, secured)) {
                TagEncoder.encode(bao, TagNum.Nested3PartyRole, nested3PartyRole.getValue());
            }
            if (noNested3PartySubIDs != null && MsgUtil.isTagInList(TagNum.NoNested3PartySubIDs, SECURED_TAGS, secured)) {
                ...
                if (nstd3PtysSubGroups != null && nstd3PtysSubGroups.length == noNested3PartySubIDs.intValue()) {
                    for (int i = 0; i < noNested3PartySubIDs.intValue(); i++) {
                        if (nstd3PtysSubGroups[i] != null) {
                            bao.write(nstd3PtysSubGroups[i].encode(getMsgSecureTypeForFlag(secured)));
                        }
                    }
                } else {
                    String error = "Nessted3PartysSubGroup field has been set but there is no data or the number of groups does not match.";
                    LOGGER.severe(error);
    ...
    throws BadFormatMsgException, InvalidMsgException, TagNotPresentException {
        if (NSTD3_PARTY_SUB_GROUP_TAGS.contains(tag.tagNum)) {
            if (noNested3PartySubIDs != null && noNested3PartySubIDs.intValue() > 0) {
                ...
                for (int i = 0; i < nstd3PtysSubGroups.length; i++) {
                    Nested3PartySubGroup group = new Nested3PartySubGroup44(context);
    ...
    protected String getUnsupportedTagMessage() {
        return "This tag is not supported in [Nested3PartyGroup] group version [4.4].";
    }

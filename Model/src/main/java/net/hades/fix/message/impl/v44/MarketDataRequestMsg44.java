...
/*
 * MarketDataRequestMsg44.java
 *
 * $Id: MarketDataRequestMsg44.java,v 1.11 2011-04-14 23:44:38 vrotaru Exp $
 */
...
import net.hades.fix.message.group.impl.v44.MDEntryTypeGroup44;
import net.hades.fix.message.group.impl.v44.MDReqGroup44;
...
/**
 * FIX version 4.4 MarketDataRequestMsg implementation.
 *
...
public class MarketDataRequestMsg44 extends MarketDataRequestMsg {
    ...
    protected static final Set<Integer> MD_ENTRY_TYPE_GROUP_TAGS = new MDEntryTypeGroup44().getFragmentAllTags();
    protected static final Set<Integer> MD_REQ_GROUP_TAGS = new MDReqGroup44().getFragmentAllTags();
    ...
    public MarketDataRequestMsg44() {
        super();
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }
    
    public MarketDataRequestMsg44(Header header, ByteBuffer rawMsg)
        throws InvalidMsgException, TagNotPresentException, BadFormatMsgException {
        super(header, rawMsg);
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }

    public MarketDataRequestMsg44(BeginString beginString) throws InvalidMsgException {
        super(beginString);
        SECURED_TAGS = STANDARD_SECURED_TAGS;
    }
    
    public MarketDataRequestMsg44(BeginString beginString, ApplVerID applVerID) throws InvalidMsgException {
    ...
    public void copyFixmlData(FIXFragment fragment) {
        MarketDataRequestMsg44 fixml = (MarketDataRequestMsg44) fragment;
    ...
    public void setNoMDEntryTypes(Integer noMDEntryTypes) {
        ...
        if (noMDEntryTypes != null) {
            ...
            for (int i = 0; i < mdEntryTypeGroups.length; i++) {
                mdEntryTypeGroups[i] = new MDEntryTypeGroup44(context);
            }
        }
    }
    ...
    public MDEntryTypeGroup addMdEntryTypeGroup() {
        MDEntryTypeGroup group = new MDEntryTypeGroup44(new FragmentContext(sessionCharset, messageEncoding, encryptionRequired, crypter, validateRequired));
    ...
    public void setNoRelatedSym(Integer noRelatedSym) {
        ...
        if (noRelatedSym != null) {
            ...
            for (int i = 0; i < mdReqGroups.length; i++) {
                mdReqGroups[i] = new MDReqGroup44(context);
            }
        }
    }
    ...
    public MDReqGroup addMdReqGroup() {
        MDReqGroup group = new MDReqGroup44(new FragmentContext(sessionCharset, messageEncoding, encryptionRequired, crypter, validateRequired));
    ...
    protected void setFragmentCompTagValue(Tag tag, ByteBuffer message)
        ...
        if (MD_ENTRY_TYPE_GROUP_TAGS.contains(tag.tagNum)) {
            if (noMDEntryTypes != null && noMDEntryTypes.intValue() > 0) {
                ...
                mdEntryTypeGroups = new MDEntryTypeGroup[noMDEntryTypes.intValue()];
                for (int i = 0; i < noMDEntryTypes.intValue(); i++) {
                    MDEntryTypeGroup group = new MDEntryTypeGroup44(context);
        ...
        if (MD_REQ_GROUP_TAGS.contains(tag.tagNum)) {
            if (noRelatedSym != null && noRelatedSym.intValue() > 0) {
                ...
                mdReqGroups = new MDReqGroup[noRelatedSym.intValue()];
                for (int i = 0; i < noRelatedSym.intValue(); i++) {
                    MDReqGroup group = new MDReqGroup44(context);
                    group.decode(message);
                    mdReqGroups[i] = group;
                }
            }
        ...
        if (TRADING_SESS_GROUP_TAGS.contains(tag.tagNum)) {
            if (noTradingSessions != null && noTradingSessions.intValue() > 0) {
                ...
                tradingSessionGroups = new TradingSessionGroup[noTradingSessions.intValue()];
                for (int i = 0; i < noTradingSessions.intValue(); i++) {
                    TradingSessionGroup44 group = new TradingSessionGroup44(context);
                    group.decode(message);
    ...
    protected String getUnsupportedTagMessage() {
        return "This tag is not supported in [MarketDataRequestMsg] message version [4.4].";
    }

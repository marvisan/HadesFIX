 ...
 *
 * $Id: ShutdownCommand.java,v 1.3 2011-03-21 04:53:03 vrotaru Exp $
 */
...
public class ShutdownCommand extends Command {
    ...
    private static final String MBEAN_OPERATION_NAME = "shutdown";
    ...
    public ShutdownCommand(String[] args) throws SessionException {
        super(args);
    }
       
    public ShutdownCommand(String[] args, MBeanServerConnection conn) throws SessionException {
        super(args, conn);
    }
        ...
        if (args.length == 1 && HELP_OPTION.equalsIgnoreCase(args[0])) {
            ...
            if (args.length != 0) {
                ...
                if (cmdResult.getOutcome()) {
                    cmdLineResult.addRow("Successfully shutdown all the instance sessions.");
                } else {
    ...
    public String getHelpText() {
        ...
        formatter.format("%1$s %n", CommandName.SERV_SHUTDOWN.getValue());
        formatter.format("\t\t%s %n", "Attempts to shutdown gracefully all the session's instances.");

...
/*
 * SetRxSeqCommand.java
 *
 * $Id: SetRxSeqCommand.java,v 1.7 2011-04-01 05:04:23 vrotaru Exp $
 */
...
/**
 * Sets the RX sequence on the server.
 * 
 * @author <a href="mailto:support@marvisan.com">Support Team</a>
 * @version $Revision: 1.7 $
 * @created 08/05/2010
 */
public class SetRxSeqCommand extends Command {

    // <editor-fold defaultstate="collapsed" desc="Constants">

    private static final String MBEAN_OPERATION_NAME = "setRxSeq";
    ...
    public SetRxSeqCommand(String[] args) throws SessionException {
        super(args);
    }
    
    public SetRxSeqCommand(String[] args, MBeanServerConnection conn) throws SessionException {
        super(args, conn);
    }
        ...
        if (args.length == 1 && HELP_OPTION.equalsIgnoreCase(args[0])) {
            ...
            if (args.length != 2) {
                ...
                if (cmdResult.getOutcome()) {
                    cmdLineResult.addRow("Sequence number [" + args[1] + "] successfully set for incoming messages on session ["
    ...
    public String getHelpText() {
        ...
        formatter.format("%1$s <session_id> <new_seq_num> %n", CommandName.SET_RX_SEQ.getValue());
        ...
        formatter.format("\t\t%1$10s - %2$s %n", "<new_seq_num>", "New expected incoming sequence number.");
        formatter.format("Note: The incoming expected sequence number cannot be set if the session is in RUNNING state. %n %n");

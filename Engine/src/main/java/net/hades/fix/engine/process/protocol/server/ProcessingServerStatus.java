...
/*
 * ProcessingServerStatus.java
 *
 * $Id: ProcessingServerStatus.java,v 1.34 2011-04-07 09:57:51 vrotaru Exp $
 */
...
/**
 * Main state responsible with processing messages on RX and TX channels.
 * 
 * @author <a href="mailto:support@marvisan.com">Support Team</a>
 * @version $Revision: 1.34 $
 */
public class ProcessingServerStatus extends Status {

    private static final Logger LOGGER = Logger.getLogger(ProcessingServerStatus.class.getName());

    private static final ProtocolState STATE = ProtocolState.PROCESSING;

    public ProcessingServerStatus(StateProcessor stateProcessor) {
        ...
        try {
            if (byteFixMsg != null && byteFixMsg.length > 0) {
                ...
                if (maxMsgSize > 0 && byteFixMsg.length > maxMsgSize) {
                    ...
                    LOGGER.log(Level.WARNING, "Discarded message with size [{0}].", byteFixMsg.length);
                    return status;
                }
                ...
                if (ProtocolState.INITIALISED.equals(processingStage)) {
                    ...
                    if (!(MsgType.Heartbeat.getValue().equals(msg.getHeader().getMsgType())
                        ...
                        ((RejectSendStatus) status).setError(new Exception("Server protocol blocked to process messages."));
                        // increment RX sequence number anyway
    ...
    private Status processNonGapMessage(FIXMsg msg) throws UnrecoverableException, InvalidMsgException, BadFormatMsgException, InterruptedException {
        ...
        if (!(MsgType.SequenceReset.getValue().equals(msg.getHeader().getMsgType()) || MsgType.Logon.getValue().equals(msg.getHeader().getMsgType()))) {
            ...
            if (msgSeqNo == expSeqNo) {
                ...
                if (MsgType.Logout.getValue().equals(msg.getHeader().getMsgType())) {
                    // if we get a Logout we don't care about sequence, just respond back and restart the session
                ...
                status.recycle();
                status.setMessage(msg);
                ((LogoutSendServerStatus) status).setText(errMsg);
                if(!MsgType.Logout.getValue().equals(msg.getHeader().getMsgType())) {
        ...
        if (msgSeqNo > expSeqNo) {
            if (possDuplicate) {
                if (isMsgSeqIsInGap(msgSeqNo, seqGap)) {
                    ...
                    LOGGER.log(Level.FINE, "Added message out of gap [{0},{1}] with sequence [{2}] to re=sequencing buffer",
    ...
    private Status processSequencedMessage(FIXMsg msg) throws InvalidMsgException, BadFormatMsgException, InterruptedException {
        ...
        if (LOGGER.isLoggable(Level.FINEST)) {
            LOGGER.log(Level.FINEST, "{0} --> {1} [{2}] : {3}", new Object[] { stateProcessor.getProtocol().getCptyID(), stateProcessor.getProtocol().getLocalID(),
            msg.getHeader().getMsgType(), msg.toString() });
        }

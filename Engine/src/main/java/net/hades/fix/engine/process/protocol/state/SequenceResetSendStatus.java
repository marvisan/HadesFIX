/*
 *   Copyright (c) 2006-2016 Marvisan Pty. Ltd. All rights reserved.
 *               Use is subject to license terms.
 */

/*
 * SequenceResetSendStatus.java
 *
 * $Id: SequenceResetSendStatus.java,v 1.8 2011-04-03 08:00:05 vrotaru Exp $
 */
package net.hades.fix.engine.process.protocol.state;

import net.hades.fix.engine.exception.UnrecoverableException;
import net.hades.fix.engine.mgmt.alert.Alert;
import net.hades.fix.engine.mgmt.alert.AlertCode;
import net.hades.fix.engine.mgmt.alert.BaseSeverityType;
import net.hades.fix.engine.mgmt.alert.ComponentType;
import net.hades.fix.engine.process.event.AlertEvent;
import net.hades.fix.engine.process.protocol.MessageFiller;
import net.hades.fix.engine.process.protocol.ProtocolState;
import net.hades.fix.engine.process.session.SessionType;
import net.hades.fix.message.SequenceResetMsg;
import net.hades.fix.message.exception.InvalidMsgException;

import java.util.logging.Logger;

/**
 * Processing state in which the client sends an SequenceReset message. <br/>
 * The outcome states are:
 * <ul>
 *      <li>PROCESSING - in case of success/error
 * </ul>
 *
 * @author <a href="mailto:support@marvisan.com">Support Team</a>
 * @version $Revision: 1.8 $
 */
public class SequenceResetSendStatus extends Status {
    
    private static final Logger LOGGER = Logger.getLogger(SequenceResetSendStatus.class.getName());

    private static final ProtocolState STATE = ProtocolState.SEQUENCE_RESET_SEND;

    private Integer newSeqNo;

    public SequenceResetSendStatus(StateProcessor stateProcessor) {
        super(stateProcessor);
    }

    @Override
    public Status process() throws UnrecoverableException, InterruptedException {
        Status status;
        try {
            status = sendSequenceReset();
        } catch (InvalidMsgException ex) {
            status = handleBuildMessageError(ex);
        }

        return status;
    }

    @Override
    public String getName() {
        return stateProcessor.getProtocol().getName() + "_" + ProtocolState.SEQUENCE_RESET_SEND.name();
    }

    @Override
    public void recycle() {
        message = null;
        newSeqNo = null;
    }

    @Override
    public ProtocolState getProtocolState() {
        return STATE;
    }

    private Status sendSequenceReset() throws InvalidMsgException, InterruptedException {
        if (newSeqNo != null) {
            SequenceResetMsg msg = MessageFiller.buildSequenceResetMsg(stateProcessor.getProtocol());
            msg.setNewSeqNo(newSeqNo);
            stateProcessor.sendProtocolMessage(msg);
        } else {
            throw new InvalidMsgException("NewSeqNo has not been set.");
        }

        return stateProcessor.getStatus(ProtocolState.PROCESSING);
    }

    private Status handleBuildMessageError(Exception ex) throws InterruptedException {
        String msg = "Invalid message : " + ex.getMessage();
        if (SessionType.BUY.equals(sessionType)) {
            stateProcessor.getProtocol().getEventProcessor().onAlertEvent(new AlertEvent(this,
                    Alert.createAlert(getName(), ComponentType.FIXClient.toString(),
                    BaseSeverityType.RECOVERABLE, AlertCode.MSG_FORMAT_ERROR, ex.toString(), ex)));
        } else {
            stateProcessor.getProtocol().getEventProcessor().onAlertEvent(new AlertEvent(this,
                    Alert.createAlert(getName(), ComponentType.FIXServer.toString(),
                    BaseSeverityType.RECOVERABLE, AlertCode.MSG_FORMAT_ERROR, ex.toString(), ex)));
        }
        LOGGER.severe(msg);
        
        return stateProcessor.getStatus(ProtocolState.PROCESSING);
    }
}

...
/*
 * FIXServerStateProcessor.java
 *
 * $Id: FIXServerStateProcessor.java,v 1.5 2011-04-30 04:39:45 vrotaru Exp $
 */
 ...
 */
public class FIXServerStateProcessor extends StateProcessor {

    private static final Logger LOGGER = Logger.getLogger(FIXServerStateProcessor.class.getName());

    public FIXServerStateProcessor(String name, Protocol protocol, Transport transport) {
        super(name, protocol, transport);
    }

    @Override
    public void run() {
        LOGGER.log(Level.INFO, "Server state processor thread [{0}] started.", getName());

        initialiseThreadSessionContextData();
        setStatus(ProtocolState.PROCESSING);
        startTimeoutTimers();
        while (!shutdown) {
            try {
                if (!blocked) {
                ...
                LOGGER.log(Level.WARNING, "Server state processor thread [{0}] interrupted.", getName());
        ...
        LOGGER.log(Level.INFO, "Server state processor thread [{0}] stopped.", getName());
    }
        ...
        try {
            if (result == null) {
                ...
                switch (protocolState) {
                    case PROCESSING:
                        result = new ProcessingServerStatus(this);
                        result.setMaxMsgSize(this.getMaxMsgSize());
                        break;

                    case LOGON_SEND:
                        result = new LogonSendServerStatus(this);
                        break;

                    case LOGON_RECEIVE:
                        result = new LogonReceiveServerStatus(this);
                        break;
                    ...
                    case LOGOUT_SEND:
                        result = new LogoutSendServerStatus(this);
                        break;

                    case LOGOUT_RECEIVE:
                        result = new LogoutReceiveServerStatus(this);
                        break;
